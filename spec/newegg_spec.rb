...
describe Newegg::Stores do
  before(:each) do
    FakeWeb.clean_registry
  end

  it %q{returns success for retrieve} do
    stores = Newegg::Stores.new
    stores.send(:retrieve).response.code.should == "200"
  end
...
describe Newegg::Categories do
  ...
  it %q{returns success for retrieve(store_id)} do
    ...
    categories.send(:retrieve, response["StoreID"]).response.code.should == "200"
  end
...
describe Newegg::Navigation do
  ...
  it %q{returns success for retrieve(store_id, category_id, node_id)} do
    ...
    navigation.send(:retrieve, response["StoreID"], response["CategoryID"], response["NodeId"]).response.code.should == "200"
  end
...
describe Newegg::Search do
  ...
  it %q{returns success for retrieve(store_id, category_id, sub_category_id, node_id)} do
    ...
    search.send(:retrieve, response["StoreID"], response["CategoryType"], response["CategoryID"], response["NodeId"], 1).response.code.should == "200"
  end
